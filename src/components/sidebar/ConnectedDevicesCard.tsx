    ...
    <div className="w-full">
      ...
      <div className="space-y-2.5">
        {devices.map((device) => (
          <div
            key={device.id}
            className="flex items-center justify-between p-3 rounded-[14.4px] bg-[#F9FAFB] border-[1.2px] border-[#F3F4F6] min-h-[56px]"
          >
            <div className="flex items-center gap-2">
              <div className="w-[33.4px] h-[33.6px] rounded-full bg-[#F3E8FF] flex items-center justify-center shrink-0">
                {getDeviceIcon(device.type)}
              </div>
              <span className="text-[clamp(13px,1.4vw,14.4px)] font-medium text-[#374151]">
                {device.name}
              </span>
            </div>

            <div className="flex items-center gap-1.5">
              <span className="text-[clamp(11px,1.2vw,12px)] font-semibold text-[#16A34A]">
                {device.syncedTime}
              </span>
